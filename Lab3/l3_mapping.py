...
class OccupancyGripMap:
    ...
    def scan_cb(self, scan_msg):
        ...
        for i in range(0, len(scan_msg.ranges), SCAN_DOWNSAMPLE):
            ...
            if np.isnan(range_mes) or np.isinf(range_mes):
                continue
            # Angle of this ray in map frame
    ...
    def ray_trace_update(self, map, log_odds, x_start, y_start, angle, range_mes):
        ...
        # Compute endpoint of ray in pixel coordinates
        ...
        # Get all pixels along the ray
        width, height = log_odds.shape
        x_end_clamped = np.clip(x_end, 0, width - 1)
        y_end_clamped = np.clip(y_end, 0, height - 1)
        x_start_clamped = np.clip(x_start, 0, width - 1)
        y_start_clamped = np.clip(y_start, 0, height - 1)


        rr, cc = ray_trace(y_start_clamped, x_start_clamped, y_end_clamped, x_end_clamped)


        # Free cells: all pixels along ray except last NUM_PTS_OBSTACLE
        free_end = max(0, len(rr) - NUM_PTS_OBSTACLE)
        if free_end > 0:
            log_odds[rr[:free_end], cc[:free_end]] -= BETA


        # Occupied cells: last NUM_PTS_OBSTACLE pixels (only if ray endpoint is in bounds)
        if 0 <= x_end < width and 0 <= y_end < height:
            log_odds[rr[free_end:], cc[free_end:]] += ALPHA


        np.clip(log_odds, -3.0, 10.0, out=log_odds)
       
        # Convert log odds to probability, then to 0-100 int8
        probs = self.log_odds_to_probability(log_odds)
        map = (probs * 100).astype(np.int8)
