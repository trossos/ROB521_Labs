...
class wheelBaselineEstimator():
    ...
    def startStopCallback(self, msg):
        if self.isMoving is False and np.absolute(msg.angular.z) > 0:
            ...
            # Wheel arc lengths
            s_l = WHEEL_RADIUS * 2 * np.pi * (self.del_left_encoder / TICKS_PER_ROTATION)
            s_r = WHEEL_RADIUS * 2 * np.pi * (self.del_right_encoder / TICKS_PER_ROTATION)


            # Total robot rotation
            delta_theta = 2 * np.pi * NUM_ROTATIONS


            # Baseline estimate
            separation = (s_l - s_r) / delta_theta
