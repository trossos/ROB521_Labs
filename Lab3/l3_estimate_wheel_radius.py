...
class wheelRadiusEstimator():
    ...
    def startStopCallback(self, msg):
        ...
        if self.isMoving is False and np.absolute(input_velocity_mag) > 0:
            ...
            delta_phi_l = (2*np.pi * self.del_left_encoder) / TICKS_PER_ROTATION
            delta_phi_r = (2*np.pi * self.del_right_encoder) / TICKS_PER_ROTATION


            # Average rotation
            delta_phi_avg = (delta_phi_l + delta_phi_r) / 2


            # Wheel radius estimate
            radius = DRIVEN_DISTANCE / delta_phi_avg
