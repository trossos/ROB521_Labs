...
class WheelOdom:
    ...
    def sensor_state_cb(self, sensor_state_msg):
        ...
        if self.last_enc_l is None:
            ...
            # Distance for each wheel
            dL = delta_le * WHEEL_RADIUS
            dR = delta_re * WHEEL_RADIUS
            ...
            delta_theta = (dR - dL) / (BASELINE * 2)
            theta_prev = euler_from_ros_quat(self.pose.orientation)[2]
            theta = theta_prev + delta_theta


            dx = d_centre * np.cos(theta_prev + delta_theta / 2)
            dy = d_centre * np.sin(theta_prev + delta_theta / 2)
            ...
            self.twist.linear.x = d_centre / dt
